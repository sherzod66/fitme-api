...
import { sendNotification } from "../firebase/firebase";
...
export class TrainerController {
  ...
  public async findByEmail(req: Request, res: Response, next: NextFunction) {
    try {
      const found = await TrainerService.find({
        phoneNumber: req.params.email,
      });
  ...
  public async requestAddTrainer(
    ...
    try {
      ...
      const foundTrainerToken: UserDocument | null = await UserModel.findOne({
        phoneNumber: foundTrainer.phoneNumber,
      });

      if (!foundTrainerToken) {
      ...
      try {
        await sendNotification(
          foundTrainerToken.messageToken,
          "Новая заявка",
          `Пользователь ${foundUser.name}, хочет заниматься у вас`
        );
      } catch (error) {
  ...
  public async updateImage(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const found = await TrainerModel.findOne({ _id: req.params.id });
      console.log(found?.avatar);
      if (!found) {
        throw createHttpError(StatusCodes.NOT_FOUND, "Exercise not found");
      }
      fs.access(
        `${path.join(path.resolve(), "static")}${found.avatar}`,
        fs.constants.F_OK,
        async (err) => {
          if (err) {
      ...
      const updated = await TrainerModel.updateOne({ _id }, { avatar: image });
      const found2 = await TrainerModel.findOne({ _id: req.params.id });
