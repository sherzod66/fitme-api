...
interface IUserIdNutritionPlan {
...
const NutritionPlanService = {
  ...
  delete: async (id: string) => {
    ...
    if (findAllUsers) {
      const allNutritionPlan: IUserIdNutritionPlan[] = [];
      findAllUsers.forEach((item, index) => {
        if (item.nutritionPlans) {
          if (item.nutritionPlans.length > 0) {
            item.nutritionPlans.forEach((elem) => {
              if (elem.toString() === id) {
                allNutritionPlan.push({
                  nutritionPlanId: elem.toString(),
                  userId: item._id,
                  index,
                });
              }
      ...
      for (const item of allNutritionPlan) {
        const currentUser = { ...findAllUsers[item.index] };
        if (currentUser.nutritionPlans) {
          const findIndex = currentUser.nutritionPlans.findIndex(
            (elem) => elem.toString() === item.nutritionPlanId
          );
          currentUser.nutritionPlans.splice(findIndex, 1);
          await UserModel.updateOne(
            { _id: item.userId },
            { $set: { nutritionPlans: currentUser.nutritionPlans } }
          );
        }
      }
    }
