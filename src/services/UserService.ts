...
const UserService = {
  ...
  addNutritionPlanByStudents: async (req: Request) => {
    // @ts-ignore
    let foundUser: IUser = req.user;
    ...
    const addPlans = await Promise.all(
      users.map(async (item) => {
        ...
        if (!currentUser.nutritionPlans?.length) return;
        const includePlan = currentUser.nutritionPlans.find(
          (elem) => elem._id.toString() === req.body.planId
        );
        if (includePlan) {
          console.log("plan exist");
          return;
        }
        console.log("plan no exist");

        currentUser.nutritionPlans = [
          ...(currentUser.nutritionPlans ?? []),
          foundPlan,
        ];
        await UserModel.findByIdAndUpdate(item, currentUser);
        await WorkoutPlanModel.findByIdAndUpdate(req.body.planId, {
          $push: { users: currentUser._id },
        });
  ...
  addWorkoutPlanByStudent: async (req: Request) => {
    // @ts-ignore
    let foundUser: IUser = req.user;
    const users: string[] = req.body.users;
    if (req.params.id !== foundUser._id.toString()) {
    ...
    const addPlans = await Promise.all(
      users.map(async (item) => {
        ...
        if (!currentUser.workoutPlans?.length) return;
        const includePlan = currentUser.workoutPlans.find(
          (elem) => elem._id.toString() === req.body.planId
        );
        if (includePlan) {
          console.log("plan exist");
  ...
  setSchemaNutrition: async (req: Request) => {
    ...
    // if (
    //   foundUser.schemaNutritions[foundUser.schemaNutritions.length - 1].data
    //     .nType !== nType
    // ) {
    //   obj.data.amount = 0;
    //   obj.data.proteinPercent = 0;
    //   obj.data.oilPercent = 0;
    //   obj.data.mergeCarb = 0;
    // }
