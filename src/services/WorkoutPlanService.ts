...
import { ScheduleWorkout, UserModel } from "../database/models/user";
...
interface IUserIdScheduleWorkout extends ScheduleWorkout {
...
const WorkoutPlanService = {
  ...
  delete: async (id: string) => {
    ...
    if (findAllUsers) {
      const allScheduleWorkouts: IUserIdScheduleWorkout[] = [];
      findAllUsers.forEach((item) => {
        if (item.scheduleWorkouts) {
          if (item.scheduleWorkouts.length > 0) {
            const workout = item.scheduleWorkouts[0];
            allScheduleWorkouts.push({
              userId: item._id,
              activeWeek: workout.activeWeek,
      ...
      for (const item of allScheduleWorkouts) {
        if (item.plan.toString() === id) {
          await UserModel.updateOne(
            { _id: item.userId },
            { $set: { scheduleWorkouts: [] } }
          );
        }
      }
    }
